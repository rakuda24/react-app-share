...
// Firebaseアプリの初期化とストレージ、Firestoreのインスタンス作成
const firebaseApp = initializeApp(firebaseConfig);
const storage = getStorage(firebaseApp);
const firestore = getFirestore(firebaseApp);
...
const ImageUploader = () => {
  ...
  const OnFileUploadToFirebase = async (e) => {
    const file = e.target.files[0];
    const storageRef = ref(storage, `test_image/${file.name}`);
    ...
    try {
      ...
      let collectionName = "defaultCollection"; // デフォルトのコレクション名

      // URLに応じてコレクション名を設定
      if (location.pathname === "/room1.html") {
        collectionName = "room1Collection";
      } else if (location.pathname === "/room2.html") {
        collectionName = "room2Collection";
      } else if (location.pathname === "/room3.html") {
        collectionName = "room3Collection";
      } else if (location.pathname === "/room4.html") {
        collectionName = "room4Collection";
      }

      // Firestoreの指定されたコレクションに新しいドキュメントを追加
  ...
  const handleSendChat = () => {
    let chatRoom = "defaultChatRoom"; // デフォルトのチャットルーム
    ...
    // チャットの送信処理（例としてnavigateでURLに遷移する）
    // 本来はチャット送信のための関数やコンポーネントを呼び出すべきです
  ...
  useEffect(() => {
    ...
    switch (location.pathname) {
      // ... 他のケースに対する処理も追加
      case "/room1.html":
        // /room1.html の場合の特別な処理
        console.log("Entered room 1");
        break;
      // ...他のケースに対する処理も追加
      default:
        // 上記以外の場合の処理
        console.log("Entered a different room");
        break;
    }
  }, [location.pathname]);
